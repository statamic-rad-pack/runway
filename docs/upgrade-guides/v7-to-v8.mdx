---
title: 'Upgrading from v7.x to v8.0'
sidebarTitle: 'v7.x to v8.0'
---

## Overview

<Warning>Please don't upgrade multiple major versions at once (eg. from v6 to v8). You should upgrade one major version at a time.</Warning>
...
1. Instead of using `->` to separate the column name and the JSON key in field handles, you should now use an underscore:
    ```yaml
    -
      handle: address->street_name # [!code --]
      handle: address_street_name # [!code ++]
      field:
        type: text
        display: 'Street Name'
   ```
